...
spec = do
  ...
  describe "transact" $ do
    it "Tries to select something that's not an identifier" $ do
     res <- catchException (transact defaultTestSettings testCradle ["./test/testdata/Transact/B.hs","5","1"])
     -- let res = "foo"
    ...
    it "Selects an identifier, for the case of a simple MVar created but never used." $ do
     transact defaultTestSettings testCradle ["./test/testdata/Transact/B.hs","24","3"]
    ...
    it "Selects an identifier, for the case of a simple MVar which is not used as an MVar." $ do
     transact defaultTestSettings testCradle ["./test/testdata/Transact/SimpleConfusion.hs","12","17"]
